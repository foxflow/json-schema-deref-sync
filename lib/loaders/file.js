...
module.exports = function (refValue, options) {
  ...
  try {
    ...
    if (window && window.schemas) { 
      const pathToAddress = function(p) {
        return `['` + p.replace(/\//g,"']['") + `']`;
      }
      var internalPath = pathToAddress(`_${filePath.replace(/^\/?_?/,'')}`)
      newValue = _.get(window.schemas,internalPath) 
    }
    
    else data = fs.readFileSync(filePath)
